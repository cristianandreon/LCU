...
int _modbus_rtu_send_msg_pre(uint8_t *req, int req_length)
{
    ...
    uint16_t crc2  = CRC16_2(req, req_length);
    uint16_t crc3  = ModRTU_CRC(req, req_length);
            
    req[req_length++] = crc >> 8;
    req[req_length++] = crc & 0x00FF;

    // req[req_length++] = crc3 & 0x00FF;
    // req[req_length++] = crc3 >> 8;
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    // memcpy(&tios, &ctx_rtu->old_tios, sizeof(struct termios));
    ...
    */
    ...
    /* Raw input */
    tios.c_lflag &= ~(ICANON | ECHO | ECHOE | ECHOK | ECHONL | ECHOCTL | ECHOPRT | ECHOKE | ISIG);
