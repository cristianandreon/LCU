...
void _error_print(modbus_t *ctx, const char *context, const char *req, const char *rsp)
{
    if (ctx->debug || ctx->debug2) {
        if (req && rsp) {
            fprintf(stderr, "[%s][Req:%1x.%1x.%1x.%1x.%1x.%1x.%1x.%1x][Resp:%1x.%1x.%1x.%1x.%1x.%1x.%1x.%1x]", context, 
                    (int)req[0], (int)req[1], (int)req[2], (int)req[3], (int)req[4], (int)req[5], (int)req[6], (int)req[7], 
                    (int)rsp[0], (int)rsp[1], (int)rsp[2], (int)rsp[3], (int)rsp[4], (int)rsp[5], (int)rsp[6], (int)rsp[7]);
        } else if (req) {
            fprintf(stderr, "[%s][Req:%1x.%1x.%1x.%1x.%1x.%1x.%1x.%1x]", context, req[0], req[1], req[2], req[3], req[4], req[5], req[6], req[7]);
        } else if (rsp) {
            fprintf(stderr, "[%s][Resp:%1x.%1x.%1x.%1x.%1x.%1x.%1x.%1x]", context, rsp[0], rsp[1], rsp[2], rsp[3], rsp[4], rsp[5], rsp[6], rsp[7]);
        } else {
            fprintf(stderr, "[%s] ERROR %s", context, modbus_strerror(errno));
        }
        if (context != NULL) {
            fprintf(stderr, ": %s\n", context);
        } else {
        ...
        fprintf(stderr, "[%s] ERROR %s", context?context:"", modbus_strerror(errno));        
    }
}
...
int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    do {
        ...
        if (rc == -1) {
            _error_print(ctx, "send", msg, NULL);
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    while (length_to_read != 0) {
        ...
        if (rc == -1) {
            _error_print(ctx, "select", NULL, NULL);
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) {
                int saved_errno = errno;
        ...
        if (rc == -1) {
            _error_print(ctx, "read", NULL, NULL);
            if ((ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK) &&
        ...
        if (length_to_read == 0) {
            ...
            case _STEP_META:
                ...
                if ((msg_length + length_to_read) > ctx->backend->max_adu_length) {
                    errno = EMBBADDATA;
                    _error_print(ctx, "too many data", NULL, NULL);
                    return -1;
                }
                step = _STEP_DATA;
                break;
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    /* Exception code */
    if (function >= 0x80) {
        if (rsp_length == (offset + 2 + ctx->backend->checksum_length) && req[offset] == (rsp[offset] - 0x80)) {
            ...
            {   char str[256];
                sprintf(str,"station#%d - exception_code:%d", ctx->slave, exception_code);
                _error_print(ctx, str, req, rsp);
               }
            return -1;
        } else {
            errno = EMBBADEXC;
            {   char str[256];
                sprintf(str,"station#%d - Invalid Expception code %d/%d", ctx->slave, req[offset], (rsp[offset] - 0x80));
                _error_print(ctx, str, req, rsp);
               }
            return -1;
        }
...
void modbus_set_debug2(modbus_t *ctx, int boolean)
{
    ctx->debug2 = boolean;
}
